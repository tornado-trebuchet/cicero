...
class CountryMapper:
    ...
    def to_orm(domain_entity: Country) -> CountryORM:
        orm = CountryORM(id=domain_entity.id.value, country=domain_entity.country)
        return orm
    ...
    def to_domain(orm_entity: CountryORM) -> Country:
        return Country(
            ...
            institutions=[UUID(i.id) for i in getattr(orm_entity, "institutions")],
            periodisation=[UUID(p.id) for p in getattr(orm_entity, "periodisation")],
