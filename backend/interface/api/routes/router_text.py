...
from backend.application.di.di_text import (
...
from backend.interface.api.dtos.dto_text import (
    ProtocolDTO,
    SpeechDTO,
    SpeechTextDTO,
)
from backend.interface.api.mappers.mp_text import (
    protocol_to_dto,
    speech_text_to_dto,
    speech_to_dto,
)
...
@text_router.get(
    "/protocols/by_institution/{institution_id}",
    response_model=List[ProtocolDTO],
)
def get_protocols_by_institution_id(
    institution_id: str,
    use_case: Any = Depends(get_protocols_by_institution_id_use_case),
) -> List[ProtocolDTO]:
...
@text_router.get(
    "/protocols/by_institution_and_period/{institution_id}/{period_id}",
    response_model=List[ProtocolDTO],
)
...
@text_router.get(
    "/protocols/by_date_range/{start_date}/{end_date}",
    response_model=List[ProtocolDTO],
)
...
def list_protocols(
    list_protocols_use_case: Any = Depends(list_protocols_use_case),
) -> List[ProtocolDTO]:
...
@text_router.get(
    "/speeches/by_date_range/{start_date}/{end_date}",
    response_model=List[SpeechDTO],
)
def get_speeches_by_date_range(
    start_date: str,
    end_date: str,
    use_case: Any = Depends(get_speeches_by_date_range_use_case),
) -> List[SpeechDTO]:
