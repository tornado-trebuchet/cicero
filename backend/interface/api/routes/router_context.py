...
from backend.application.di.di_context import (
...
from backend.interface.api.dtos.dto_context import (
    CountryDTO,
    InstitutionDTO,
    PartyDTO,
    PeriodDTO,
...
from backend.interface.api.mappers.mp_context import (
    country_to_dto,
    institution_to_dto,
    party_to_dto,
    period_to_dto,
...
def list_countries(
    list_countries_use_case: Any = Depends(list_countries_use_case),
) -> List[CountryDTO]:
...
def list_institutions(
    list_institutions_use_case: Any = Depends(list_institutions_use_case),
) -> List[InstitutionDTO]:
...
@context_router.get(
    "/institutions/by_type/{institution_type}",
    response_model=List[InstitutionDTO],
)
def get_institutions_by_type(
    institution_type: InstitutionTypeEnum,
    use_case: Any = Depends(get_institutions_by_type_use_case),
) -> List[InstitutionDTO]:
...
def list_parties(
    list_parties_use_case: Any = Depends(list_parties_use_case),
) -> List[PartyDTO]:
...
def list_periods(
    list_periods_use_case: Any = Depends(list_periods_use_case),
) -> List[PeriodDTO]:
...
def get_periods_by_owner(
    owner_id: str,
    owner_type: str,
    use_case: Any = Depends(get_periods_by_owner_use_case),
) -> List[PeriodDTO]:
