...
class ProtocolORM(Base):
    ...
    regex_pattern_id: Mapped[Optional[uuid.UUID]] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("regex_patterns.id"), nullable=True)
    ...
    regex_pattern: Mapped[Optional["RegexPatternORM"]] = relationship(
        "RegexPatternORM",
        back_populates="protocols"
    )
