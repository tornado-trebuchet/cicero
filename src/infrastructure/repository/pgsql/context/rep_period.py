...
class PeriodRepository(IPeriodRepository):
    def get_by_id(self, id: UUID) -> Optional[Period]:
        with session_scope() as session:
            orm_period = (
                session.query(PeriodORM).filter_by(id=id.value).one_or_none()
            )
            if orm_period:
    ...
    def get_by_owner_id(self, owner_id: UUID) -> List[Period]:
        with session_scope() as session:
            orm_periods = (
                ...
                .filter_by(owner_id=owner_id.value)
                .all()
            )
    ...
    def get_by_owner(
        ...
        with session_scope() as session:
            orm_periods = (
                session.query(PeriodORM)
                .filter_by(owner_id=owner_id.value, owner_type=owner_type)
                .all()
            )
            return [PeriodMapper.to_domain(orm) for orm in orm_periods]
