from typing import Optional
from src.domain.models.v_common import UUID, DateTime, HttpUrl
from src.domain.models.base_model import Entity
from src.domain.models.v_enums import ProtocolTypeEnum, ExtensionEnum

class Protocol(Entity):
    """
    Represents a protocol (e.g., parliamentary session record) in the domain model.
    """
    def __init__(
        self,
        id: UUID,
        institution_id: UUID,
        extension: ExtensionEnum,
        protocol_type: ProtocolTypeEnum,
        date: DateTime,
        period: Optional[UUID] = None,
        file_source: Optional[HttpUrl] = None,
        speech_regex: Optional[UUID] = None,
        metadata: Optional[dict] = None,
    ):
        super().__init__(id)
        self._institution_id = institution_id
        self._period = period
        self._extension = extension
        self._file_source = file_source
        self._protocol_type = protocol_type
        self._speech_regex = speech_regex
        self._date = date
        self._metadata = metadata if metadata is not None else {}

    @property
    def institution_id(self) -> UUID:
        return self._institution_id

    @institution_id.setter
    def institution_id(self, value: UUID):
        self._institution_id = value

    @property
    def period(self) -> Optional[UUID]:
        return self._period

    @period.setter
    def period(self, value: Optional[UUID]):
        self._period = value

    @property
    def extension(self) -> ExtensionEnum:
        return self._extension

    @extension.setter
    def extension(self, value: ExtensionEnum):
        self._extension = value

    @property
    def file_source(self) -> Optional[HttpUrl]:
        return self._file_source

    @file_source.setter
    def file_source(self, value: Optional[HttpUrl]):
        self._file_source = value

    @property
    def protocol_type(self) -> ProtocolTypeEnum:
        return self._protocol_type

    @protocol_type.setter
    def protocol_type(self, value: ProtocolTypeEnum):
        self._protocol_type = value

    @property
    def speech_regex(self) -> Optional[UUID]:
        return self._speech_regex

    @speech_regex.setter
    def speech_regex(self, value: Optional[UUID]):
        self._speech_regex = value

    @property
    def date(self) -> DateTime:
        return self._date

    @date.setter
    def date(self, value: DateTime):
        self._date = value

    @property
    def metadata(self) -> dict:
        return self._metadata

    @metadata.setter
    def metadata(self, value: dict):
        self._metadata = value

    def __repr__(self) -> str:
        return f"<Protocol {self._protocol_type} {self._date} {self.id}>"
